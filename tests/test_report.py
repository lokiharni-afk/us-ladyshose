...
def test_report_builds_temu_follow_list_from_top20():
    ...
    assert "recovery slides | $35.00 | ¥71.93-¥143.93 | 中 | 高" in text
    assert "OOFOS Recovery Slides" not in text.split("## Temu跟款清单", 1)[1].split("## 风险提醒", 1)[0]
    assert "品牌词 OOFOS，跟款优先级低" in text
    assert "orthopedic arch support sandals | $25.00 | ¥57.53-¥107.93 | 低 | 高" in text
    assert "beach sandals | $12.00 | ¥50.33-¥93.53 | 低 | 中" in text
